...
EXTRACT_MESSAGE_SYSTEM = """당신은 메신저 대화 스크린샷에서 텍스트를 추출하는 전문가입니다.

이미지에서 상대방이 보낸 메시지(왼쪽 또는 회색 말풍선)의 텍스트를 그대로 추출하세요.
여러 메시지가 있다면 모두 순서대로 이어서 작성하세요.
추출한 텍스트만 응답하고, 설명이나 부가 내용은 절대 포함하지 마세요."""
...
def extract_message_from_screenshot(image_bytes: bytes, media_type: str = "image/png") -> str:
    """대화 스크린샷에서 상대방 메시지 텍스트를 추출합니다."""
    image_data = base64.standard_b64encode(image_bytes).decode("utf-8")
        ...
        messages=[
            {
                ...
                "content": [
                    ...
                    {
                        "type": "text",
                        "text": "이 스크린샷에서 상대방(왼쪽/회색 말풍선)이 보낸 메시지 텍스트만 추출해주세요.",
                    },
                ],
            }
        ],
